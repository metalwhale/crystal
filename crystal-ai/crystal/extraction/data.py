...
def _sanitize(truth_fields_str: str, origin_text: str) -> dict[str, dict[str, str]]:
    ...
    for key, info in truth_fields.items():
        if not (
            ...
            continue
        if (
            "field" in key or key in ["field_name", "description", "value"]
        ...
        if (
            ...
            or "フィールド" in description or description in ["field_name", "description", "value"]
            or str(date.today().year) in value or value in ["field_name", "description", "value"]
        ):
            continue
