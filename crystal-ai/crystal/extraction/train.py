        ...
        **kwargs,
        ...
        for text, completion, response in zip(origin_text, completions, truth_fields_str):
            ...
            if not isinstance(fields, dict):
                ...
                continue
            reward = 0
            key_reward = 1 / len(truth_fields)
            for key in fields.keys():
                if key not in truth_fields:
                    reward += key_reward * -0.5
                    continue
                if key in keys:
                    reward += key_reward * -0.5
                    continue
                ...
                if not isinstance(fields[key], str):
                    reward += key_reward * 0.2
                    continue
                value = fields[key]
                if value not in text:
                    reward += key_reward * 0.4
                    continue
                if value not in truth_fields[key]["value"]:
                    reward += key_reward * 0.6
                    continue
                if value != truth_fields[key]["value"]:
                    reward += key_reward * 0.8
                    continue
                reward += key_reward
